...
std::string GamestationSystem::readFile(std::string file) {
    if (file.size() > 0) {
...
std::vector<std::string> GamestationSystem::getAvailableWiFiSSID(bool activatedWifi) {
	if (!activatedWifi){
		enableWifi("","");
	}
	std::vector<std::string> res;
	std::ostringstream oss;
	oss << Settings::getInstance()->getString("GamestationSettingScript") << " " << "wifi list";
...
std::vector<std::string> GamestationSystem::getAvailableAudioOutputDevices() {
	std::vector<std::string> res;
	std::ostringstream oss;
	oss << Settings::getInstance()->getString("GamestationSettingScript") << " " << "lsaudio";
	FILE *pipe = popen(oss.str().c_str(), "r");
	char line[1024];
...
std::string GamestationSystem::getCurrentAudioOutputDevice() {
	...
	std::ostringstream oss;
	oss << Settings::getInstance()->getString("GamestationSettingScript") << " " << "getaudio";
	FILE *pipe = popen(oss.str().c_str(), "r");
	char line[1024];
...
bool GamestationSystem::setAudioOutputDevice(std::string selected) {
	...
	oss << Settings::getInstance()->getString("GamestationSettingScript") << " " << "audio" << " '" << selected << "'";
...
bool GamestationSystem::setOverscan(bool enable) {
    ...
    std::ostringstream oss;
    oss << Settings::getInstance()->getString("GamestationSettingScript") << " " << "overscan";
    if (enable) {
        oss << " " << "enable";
    } else {
        oss << " " << "disable";
...
bool GamestationSystem::setOverclock(std::string mode) {
    if (mode != "") {
        std::ostringstream oss;
        oss << Settings::getInstance()->getString("GamestationSettingScript") << " "
            << "overclock" << " " << mode;
        std::string command = oss.str();
        LOG(LogInfo) << "Launching " << command;
        if (system(command.c_str())) {
...
std::vector<std::string> *GamestationSystem::scanBluetooth() {
    std::vector<std::string> *res = new std::vector<std::string>();
    std::ostringstream oss;
    oss << Settings::getInstance()->getString("GamestationSettingScript") << " " << "hcitoolscan";
    FILE *pipe = popen(oss.str().c_str(), "r");
    char line[1024];
...
bool GamestationSystem::pairBluetooth(std::string &controller) {
    std::ostringstream oss;
    oss << Settings::getInstance()->getString("GamestationSettingScript") << " " << "hiddpair" << " " << controller;
    int exitcode = system(oss.str().c_str());
    return exitcode == 0;
}
...
std::vector<std::string> GamestationSystem::getAvailableStorageDevices() {
    ...
    std::vector<std::string> res;
    std::ostringstream oss;
    oss << Settings::getInstance()->getString("GamestationSettingScript") << " " << "storage list";
    FILE *pipe = popen(oss.str().c_str(), "r");
    char line[1024];
...
std::string GamestationSystem::getCurrentStorage() {
    ...
    std::ostringstream oss;
    oss << Settings::getInstance()->getString("GamestationSettingScript") << " " << "storage current";
    FILE *pipe = popen(oss.str().c_str(), "r");
    char line[1024];
...
bool GamestationSystem::setStorage(std::string selected) {
    std::ostringstream oss;
    oss << Settings::getInstance()->getString("GamestationSettingScript") << " " << "storage" << " " << selected;
    int exitcode = system(oss.str().c_str());
    return exitcode == 0;
}
...
bool GamestationSystem::forgetBluetoothControllers() {
    std::ostringstream oss;
    oss << Settings::getInstance()->getString("GamestationSettingScript") << " " << "forgetBT";
    int exitcode = system(oss.str().c_str());
    return exitcode == 0;
}
...
std::string GamestationSystem::getRootPassword() {
    ...
    std::ostringstream oss;
    oss << Settings::getInstance()->getString("GamestationSettingScript") << " " << "getRootPassword";
    FILE *pipe = popen(oss.str().c_str(), "r");
    char line[1024];
...
std::pair<std::string, int> GamestationSystem::execute(std::string command) {
    std::ostringstream oss;
    oss << command;
...
std::pair<std::string, int> GamestationSystem::getSDLBatteryInfo(){
...
std::pair<std::string, int> GamestationSystem::getSysBatteryInfo(){
	std::pair<std::string, int> result;
...
std::string GamestationSystem::getJSONStringValue(std::string json, std::string key) {
...
EmulatorDefaults GamestationSystem::getEmulatorDefaults(std::string emulatorName) {
    EmulatorDefaults defaults;
    auto initConfig = new GamestationConf(false);
...
std::string GamestationSystem::runCmd(std::string options) {
    std::string cmd = Settings::getInstance()->getString("GamestationSettingScript") + " " + options;
