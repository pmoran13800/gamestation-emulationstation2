...
NetPlayThread::NetPlayThread(Window* window, int event)
  : mWindow(window),
    ...
    mEvent(event),
    mLobby(GamestationConf::getInstance()->get("global.netplay.lobby"))
{
}
...
void NetPlayThread::run()
{
  ...
  if (popupDuration != 0)
  {
    ...
    while (mRunning)
    {
      auto json_req = GamestationSystem::getInstance()->execute("curl -s --connect-timeout 3 -m 3 " + mLobby);
      if (json_req.second == 0)
      {
        json::ptree root;
        std::stringstream ss;
        ...
        for (auto tmp : newGames)
        {
          ...
          if (it == oldGames.end())
          {
            if (tmp.first.find("GAMESTATION") != std::string::npos)
            {
              // Create popup text
              mLastPopupText = _("A Gamestation friend has started a Netplay game!")
                             + "\n " + _("Player") + ": " + tmp.first;
                             + "\n " + _("Game") + ": " + tmp.second;
