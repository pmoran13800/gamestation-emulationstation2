...
void ISimpleGameListView::onFileChanged(FileData* file, FileChangeType change) {
	...
	if (GamestationConf::getInstance()->getBool(getRoot()->getSystem()->getName() + ".flatfolder")) {
...
bool ISimpleGameListView::input(InputConfig* config, Input input) {
	bool hideSystemView = GamestationConf::getInstance()->get("emulationstation.hidesystemview") == "1";
	...
	if (input.value != 0) {
		...
		}else if ((config->isMappedTo("x", input)) && (GamestationConf::getInstance()->get("global.netplay") == "1")
		          && (GamestationConf::getInstance()->isInList("global.netplay.systems", getCursor()->getSystem()->getName())))
		{
			FileData* cursor = getCursor();
			if(cursor->getType() == GAME)
			{
