...
bool SystemView::input(InputConfig* config, Input input)
{
	if(input.value != 0)
	{
        ...
        if(	config->isMappedTo("x", input) && input.value) {
		    bool kodiEnabled = GamestationConf::getInstance()->get("kodi.enabled") == "1";
		    bool kodiX = GamestationConf::getInstance()->get("kodi.xbutton") == "1";
		    bool netplay = GamestationConf::getInstance()->get("global.netplay") == "1";
		    ...
		    if (kodiEnabled && kodiX && !launchKodi && !mWindow->isShowingPopup()) {
		        if (netplay) {
                    ...
                    row.makeAcceptInputHandler([this, s] {
                        launchKodi = true;
                        if( ! GamestationSystem::getInstance()->launchKodi(mWindow)) {
		...
		if(config->isMappedTo("select", input) && GamestationConf::getInstance()->get("emulationstation.menu") != "none")
		{
...
std::vector<HelpPrompt> SystemView::getHelpPrompts()
 {
	...
	prompts.push_back(HelpPrompt("b", _("SELECT")));
	 if (GamestationConf::getInstance()->get("kodi.enabled") == "1" && GamestationConf::getInstance()->get("kodi.xbutton") == "1") {
		 if (GamestationConf::getInstance()->get("global.netplay") == "1") {
			 prompts.push_back(HelpPrompt("x", _("KODI/NETPLAY")));
		 } else {
			 prompts.push_back(HelpPrompt("x", _("START KODI")));
		 }
	 } else if (GamestationConf::getInstance()->get("global.netplay") == "1") {
