...
{
  mSortId = GamestationConf::getInstance()->getUInt(mName + ".sort");
  ...
  if (GamestationConf::getInstance()->get("emulationstation.gamelistonly") != "1")
...
void SystemData::launchGame(Window* window, FileData* game, std::string netplay, std::string core, std::string ip, std::string port)
{
  ...
  command = strreplace(command, "%RATIO%", game->metadata.get("ratio"));
	...
	else if (netplay == "host")
	{
		...
		command = strreplace(command, "%NETPLAY%", "-netplay host -netplay_port " + GamestationConf::getInstance()->get("global.netplay.port") + hashcmd);
	}
	else
	{
