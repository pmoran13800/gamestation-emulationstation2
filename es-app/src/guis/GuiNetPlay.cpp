...
{
	mRooms.clear();
	std::string lobby = GamestationConf::getInstance()->get("global.netplay.lobby");
	auto json_req = GamestationSystem::getInstance()->execute("curl -s --connect-timeout 3 -m 3 " + lobby);
...
void GuiNetPlay::populateGridMeta(int i)
{
    ...
    if (std::regex_search(username, std::regex("@GAMESTATION")))
    {
        username = std::regex_replace(username, std::regex("@GAMESTATION"), " \uF200");
    }
    else if (std::regex_search(frontend, std::regex("@GAMESTATION")))
    {
        username = username + " \uF200";
		frontend = std::regex_replace(frontend, std::regex("@GAMESTATION"), "");
    }
...
FileData* GuiNetPlay::findGame(std::string gameNameOrHash)
{
	for (auto tmp : SystemData::sSystemVector) {
		if (GamestationConf::getInstance()->isInList("global.netplay.systems", tmp->getName())) {
...
std::pair<std::string, std::string> GuiNetPlay::getCoreInfo(const std::string &name) {
    ...
    std::string line;
    std::string filePath = "/gamestation/share/system/configs/retroarch.corenames";
...
{
    std::pair<std::string, int> ping = GamestationSystem::getInstance()->execute("ping -c 1 -w 1 " + ip + " | grep \"min/avg/max\" | cut -d '/' -f 5");
    if (ping.first != "") {
