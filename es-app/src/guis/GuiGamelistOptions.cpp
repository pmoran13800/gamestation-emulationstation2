...
GuiGamelistOptions::GuiGamelistOptions(Window* window, SystemData* system) :
	...
	addSaveFunc([this, system] { GamestationConf::getInstance()->setUInt(system->getName() + ".sort", (unsigned int) mListSort->getSelected()); });
	...
	flat_folders->setState(GamestationConf::getInstance()->getBool(system->getName() + ".flatfolder"));
	mMenu.addWithLabel(flat_folders, _("SHOW FOLDERS CONTENT"), _(MenuMessages::GAMELISTOPTION_SHOW_FOLDER_CONTENT_MSG));
	addSaveFunc([flat_folders, system] { GamestationConf::getInstance()->setBool(system->getName() + ".flatfolder", flat_folders->getState()); });

	// edit game metadata
	row.elements.clear();

	if (GamestationConf::getInstance()->get("emulationstation.menu") != "none" && GamestationConf::getInstance()->get("emulationstation.menu") != "bartop"){
