...
GuiHashStart::GuiHashStart(Window* window) : GuiComponent(window), mMenu(window, _("HASH NOW").c_str()), mBusyAnim(window)
{
    ...
    {
        if(GamestationConf::getInstance()->isInList("global.netplay.systems", (*it)->getName()))
...
void GuiHashStart::start()
{
    for(auto system : mSystems->getSelectedObjects()) {
    	...
    	std::string command = "/gamestation/scripts/gamestation-hash.sh -s \"" + system->getName() + "\" -t";

	    auto cmdResult = GamestationSystem::getInstance()->execute(command);
        ...
        for(pugi::xml_node fileNode = root.child(tag); fileNode; fileNode = fileNode.next_sibling(tag))
        {
            ...
            std::string cmd = "/gamestation/scripts/gamestation-hash.sh -f \"" + path.string() + "\"";

	        auto hashResult = GamestationSystem::getInstance()->execute(cmd);
