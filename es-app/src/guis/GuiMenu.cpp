...
GuiMenu::GuiMenu(Window* window) : GuiComponent(window), mMenu(window, _("MAIN MENU").c_str()), mVersion(window) {
	...
	bool bartop =  GamestationConf::getInstance()->get("emulationstation.menu") == "bartop";

	//KODI
    if (GamestationConf::getInstance()->get("kodi.enabled") == "1") {
        addEntryWithHelp(_("MEDIASTATION").c_str(), _(MenuMessages::START_KODI_HELP_MSG), mMenuTheme->menuText.color, true,
                 [this] {
    ...
    mVersion.setText("RH GAMESTATION V" + strToUpper(PROGRAM_VERSION_STRING));
...
void GuiMenu::createInputTextRow(GuiSettings *gui, std::string title, const char *settingsID, bool password, std::string help = "") {
	...
	ed = std::make_shared<TextComponent>(window, ((password && GamestationConf::getInstance()->get(settingsID) != "") ? 
...
void GuiMenu::menuSystem(){
	...
	auto version = std::make_shared<TextComponent>(mWindow, 
			GamestationUpgrade::getInstance()->getVersion(), 
	...
	s->addSaveFunc([window, language_choice, language, optionsStorage, selectedStorage] {
		...
		if (language != language_choice->getSelected()) {
			GamestationConf::getInstance()->set("system.language", language_choice->getSelected());
...
void GuiMenu::menuUpdates(){
	...
	updates_enabled->setState(GamestationConf::getInstance()->get("updates.enabled") == "1");
	...
	auto updatesTypeComp = std::make_shared<OptionListComponent<std::string> >(mWindow, 
			_("UPDATE TYPE"), false);
	std::string updatesType = GamestationConf::getInstance()->get("updates.type");
	// Stable or nothing
	...
	updateGui->addSaveFunc([updates_enabled, updatesTypeComp] {
		GamestationConf::getInstance()->set("updates.enabled", updates_enabled->getState() ? "1" : "0");
		if(updatesTypeComp->getSelected() == "stable"){
			GamestationConf::getInstance()->set("updates.type", updatesTypeComp->getSelected());
		}
		GamestationConf::getInstance()->saveGamestationConf();
	});
...
void GuiMenu::menuGameSettings(){
	...
	if (GamestationConf::getInstance()->get("emulationstation.menu") != "bartop") {
		// Screen ratio choice
	...
	smoothing_enabled->setState(GamestationConf::getInstance()->get("global.smooth") == "1");
	...
	rewind_enabled->setState(GamestationConf::getInstance()->get("global.rewind") == "1");
	...
	autosave_enabled->setState(GamestationConf::getInstance()->get("global.autosave") == "1");
	...
	integerscale_enabled->setState(GamestationConf::getInstance()->get("global.integerscale") == "1");
	...
	s->addSaveFunc([integerscale_enabled] {
		GamestationConf::getInstance()->set("global.integerscale", integerscale_enabled->getState() ? "1" : "0");
		GamestationConf::getInstance()->saveGamestationConf();
	});
		...
		// Retroachievements
		{
			std::function<void()> openGui = [this] {
				...
				retroachievements_hardcore_enabled->setState(
						GamestationConf::getInstance()->get("global.retroachievements.hardcore") ==
						"1");
				...
				retroachievements->addSaveFunc(
						[retroachievements_enabled, retroachievements_hardcore_enabled] {
							GamestationConf::getInstance()->set("global.retroachievements", retroachievements_enabled->getState() ? "1" : "0");
							GamestationConf::getInstance()->set("global.retroachievements.hardcore", retroachievements_hardcore_enabled->getState() ? "1" : "0");
							GamestationConf::getInstance()->saveGamestationConf();
						});
		...
		if (GamestationConf::getInstance()->get("emulationstation.menu") != "bartop") {
			// Netplay
			{
				std::function<void()> openGui = [this] {
					...
					netplay_enabled->setState(GamestationConf::getInstance()->get("global.netplay") == "1");
					...
					std::string currentMitm = GamestationConf::getInstance()->get("global.netplay.relay");
					...
					netplay->addSaveFunc(
							[netplay_enabled, mitm_choices] {
								...
								GamestationConf::getInstance()->set("global.netplay", netplay_enabled->getState() ? "1" : "0");
								GamestationConf::getInstance()->set("global.netplay.relay", mitm);
								GamestationConf::getInstance()->saveGamestationConf();
							});
	...
	s->addSaveFunc([smoothing_enabled, rewind_enabled, shaders_choices, autosave_enabled] {
		GamestationConf::getInstance()->set("global.smooth", smoothing_enabled->getState() ? "1" : "0");
		GamestationConf::getInstance()->set("global.rewind", rewind_enabled->getState() ? "1" : "0");
		GamestationConf::getInstance()->set("global.shaderset", shaders_choices->getSelected());
		GamestationConf::getInstance()->set("global.autosave", autosave_enabled->getState() ? "1" : "0");
		GamestationConf::getInstance()->saveGamestationConf();
	});
	mWindow->pushGui(s);
}
...
void GuiMenu::menuControllers() {
					...
					for (std::vector<std::string>::iterator controllerString = ((std::vector<std::string> *) controllers)->begin();
						...
						std::function<void()> pairController = [this, window, pairGui, controllerString, deletePairGui] {
							window->pushGui(new GuiLoading(window, [controllerString] {
								bool paired = GamestationSystem::getInstance()->pairBluetooth(*controllerString);
	...
	row.makeAcceptInputHandler([window, this, s, showControllerList] {
		window->pushGui(new GuiLoading(window, [] {
			auto s = GamestationSystem::getInstance()->scanBluetooth();
			return (void *) s;
		}, showControllerList));
	});
...
void GuiMenu::menuUISettings(){
	...
	s->addSubMenu(_("UPDATE GAMES LISTS"), [this,window] {
		window->pushGui(new GuiMsgBox(window, _("DO YOU WANT TO UPDATE GAMES LISTS?"), _("YES"), [this,window] { 
				ViewController::get()->goToStart();
				window->renderShutdownScreen();
				delete ViewController::get();
...
void GuiMenu::menuSoundSettings(){
	...
	if (GamestationConf::getInstance()->get("emulationstation.menu") != "bartop") {
		...
		auto setAudioDevice = [](const std::string &newVal) {
			GamestationSystem::getInstance()->setAudioOutputDevice(newVal);
		};
	...
	s->addSaveFunc([optionsAudio, currentDevice, sounds_enabled, volume] {
		GamestationConf::getInstance()->set("audio.volume", std::to_string((int) round(volume->getValue())));

		GamestationConf::getInstance()->set("audio.bgmusic", sounds_enabled->getState() ? "1" : "0");
...
void GuiMenu::menuNetworkSettings(){
	...
	auto status = std::make_shared<TextComponent>(mWindow, GamestationSystem::getInstance()->ping() ? _( "CONNECTED")
	...
	//SSID
	std::string baseSSID = GamestationConf::getInstance()->get("wifi.ssid");
	auto WifiSSID = [this, baseSSID, enable_wifi, baseEnabled] (GuiSettings *gui, std::string title, 
		...
		row.makeAcceptInputHandler([this, updateVal, enable_wifi, baseEnabled] {
			...
			auto updateValue = [this, updateVal, SSID](const std::string &newVal) {
				GamestationConf::getInstance()->set("wifi.ssid", newVal);
				updateVal(newVal);
				delete SSID;
			};
			...
			if (enable_wifi->getState()) {
				std::vector<std::string> availableSSID = GamestationSystem::getInstance()->getAvailableWiFiSSID(baseEnabled);
				GamestationConf::getInstance()->set("wifi.enabled", "1");
	...
	s->addSaveFunc([baseEnabled, baseSSID, baseKEY, enable_wifi, window] {
		...
		else if (baseEnabled || GamestationConf::getInstance()->get("wifi.enabled") == "1"){
...
void GuiMenu::menuAdvancedSettings(){
				...
				window->pushGui(
						new GuiMsgBox(window, _("TURBO AND EXTREM OVERCLOCK PRESETS MAY CAUSE SYSTEM UNSTABILITIES, SO USE THEM AT YOUR OWN RISK.\nIF YOU CONTINUE, THE SYSTEM WILL REBOOT NOW."),
								_("YES"), [window] {
									...
									GamestationSystem::getInstance()->setOverclock("none");
									Settings::getInstance()->saveFile();
								}));
			} else {
				window->pushGui(
	...
	// BOOT
	{
		std::function<void()> openGui = [this] {
			...
			auto kodiAtStart = std::make_shared<SwitchComponent>(mWindow, GamestationConf::getInstance()->get("kodi.atstartup") == "1");
			bootGui->addWithLabel(kodiAtStart, _("KODI AT START"), _(MenuMessages::ADVANCED_KODI_AT_START_HELP_MSG));
			// Gamelists only
			bool gamelistOnly = GamestationConf::getInstance()->get("emulationstation.gamelistonly") == "1";
			...
			// Selected System
			std::string selectedsystem = GamestationConf::getInstance()->get(
			...
			// Boot on gamelist
			bool bootOnGamelist =
					GamestationConf::getInstance()->get("emulationstation.bootongamelist") == "1";
			...
			// Hide system view
			bool hidesystemview =
					GamestationConf::getInstance()->get("emulationstation.hidesystemview") == "1";
			...
			bool basicgamelistview =
					GamestationConf::getInstance()->get("emulationstation.forcebasicgamelistview") == "1";
			...
			bootGui->addSaveFunc(
					[gamelistOnlyComp, system_choices, kodiAtStart, bootOnGamelistComp, hidesystemviewComp, basicgamelistviewComp] {
						GamestationConf::getInstance()->set("kodi.atstartup", kodiAtStart->getState() ? "1" : "0");
						GamestationConf::getInstance()->set("emulationstation.gamelistonly", gamelistOnlyComp->getState() ? "1" : "0");
						GamestationConf::getInstance()->set("emulationstation.selectedsystem", system_choices->getSelected());
						GamestationConf::getInstance()->set("emulationstation.bootongamelist", bootOnGamelistComp->getState() ? "1" : "0");
						GamestationConf::getInstance()->set("emulationstation.hidesystemview", hidesystemviewComp->getState() ? "1" : "0");
						GamestationConf::getInstance()->set("emulationstation.forcebasicgamelistview", basicgamelistviewComp->getState() ? "1" : "0");
						GamestationConf::getInstance()->saveGamestationConf();
					});
	...
	{
		std::function<void()> openGui = [this] {
			GuiSettings *kodiGui = new GuiSettings(mWindow, _("MEDIASTATION SETTINGS").c_str());
			...
			kodiGui->addWithLabel(kodiEnabled, _("ENABLE MEDIASTATION"), _(MenuMessages::ADVANCED_KODI_ENABLE_HELP_MSG));
			auto kodiAtStart = std::make_shared<SwitchComponent>(mWindow, GamestationConf::getInstance()->get("kodi.atstartup") == "1");
			kodiGui->addWithLabel(kodiAtStart, _("MEDIASTATION AT START"), _(MenuMessages::ADVANCED_KODI_AT_START_HELP_MSG));
			auto kodiX = std::make_shared<SwitchComponent>(mWindow);
			kodiX->setState(GamestationConf::getInstance()->get("kodi.xbutton") == "1");
			kodiGui->addWithLabel(kodiX, _("START MEDIASTATION WITH X"), _(MenuMessages::ADVANCED_KODI_X_HELP_MSG));
			kodiGui->addSaveFunc([kodiEnabled, kodiAtStart, kodiX] {
				GamestationConf::getInstance()->set("kodi.enabled", kodiEnabled->getState() ? "1" : "0");
				GamestationConf::getInstance()->set("kodi.atstartup", kodiAtStart->getState() ? "1" : "0");
				GamestationConf::getInstance()->set("kodi.xbutton", kodiX->getState() ? "1" : "0");
				GamestationConf::getInstance()->saveGamestationConf();
			});
			mWindow->pushGui(kodiGui);
		};
		s->addSubMenu(_("MEDIASTATION SETTINGS"), openGui, _(MenuMessages::ADVANCED_KODI_HELP_MSG));
	}
	...
	//Security
	{
		std::function<void()> openGui = [this] {
			...
			securityEnabled->setState(
					GamestationConf::getInstance()->get("system.security.enabled") == "1");
			securityGui->addWithLabel(securityEnabled, _("ENFORCE SECURITY"), _(MenuMessages::ADVANCED_ENFORCE_SECURITY_HELP_MSG));

			auto rootpassword = std::make_shared<TextComponent>(mWindow, GamestationSystem::getInstance()->getRootPassword(), 
					mMenuTheme->menuText.font, mMenuTheme->menuText.color);
			...
			securityGui->addSaveFunc([this, securityEnabled] {
				...
				if (securityEnabled->changed()) {
					GamestationConf::getInstance()->set("system.security.enabled", securityEnabled->getState() ? "1" : "0");
					GamestationConf::getInstance()->saveGamestationConf();
					reboot = true;
				}
	...
	// Gamestation Manager
	auto manager = std::make_shared<SwitchComponent>(mWindow, GamestationConf::getInstance()->get("system.manager.enabled") == "1");
	s->addWithLabel(manager, _("GAMESTATION MANAGER"), _(MenuMessages::ADVANCED_MANAGER_HELP_MSG));
	s->addSaveFunc([manager] {
		GamestationConf::getInstance()->set("system.manager.enabled", manager->getState() ? "1" : "0");
		GamestationConf::getInstance()->saveGamestationConf();;
	});
	// Gamestation API
	auto gamestationApi = std::make_shared<SwitchComponent>(mWindow, GamestationConf::getInstance()->get("system.api.enabled") == "1");
	s->addWithLabel(gamestationApi, _("GAMESTATION API"), _(MenuMessages::ADVANCED_API_HELP_MSG));
	s->addSaveFunc([gamestationApi] {
		GamestationConf::getInstance()->set("system.api.enabled", gamestationApi->getState() ? "1" : "0");
		GamestationConf::getInstance()->saveGamestationConf();;
	});
	mWindow->pushGui(s);
}
...
void GuiMenu::menuQuit(){
	...
	row.addElement(std::make_shared<TextComponent>(window, _("QUICK SHUTDOWN"), mMenuTheme->menuText.font, mMenuTheme->menuText.color), true);
	s->addRow(row);
	mWindow->pushGui(s);
}
...
void GuiMenu::popSystemConfigurationGui(SystemData *systemData) const {
    ...
    EmulatorDefaults emulatorDefaults = GamestationSystem::getInstance()->getEmulatorDefaults(systemData->getName());
    ...
    std::string currentEmulator = GamestationConf::getInstance()->get(systemData->getName() + ".emulator");
    ...
    emu_choice->setSelectedChangedCallback([this, systemData, emulatorDefaults, core_choice](std::string emulatorName) {
        ...
        std::string currentCore = GamestationConf::getInstance()->get(systemData->getName() + ".core");
    ...
    // smoothing
    auto smoothing_enabled = std::make_shared<SwitchComponent>(mWindow);
    smoothing_enabled->setState(GamestationConf::getInstance()->get(systemData->getName() + ".smooth", GamestationConf::getInstance()->get("global.smooth")) == "1");
    systemConfiguration->addWithLabel(smoothing_enabled, _("SMOOTH GAMES"), _(MenuMessages::GAME_SMOOTH_HELP_MSG));
    // rewind
    auto rewind_enabled = std::make_shared<SwitchComponent>(mWindow);
    rewind_enabled->setState(GamestationConf::getInstance()->get(systemData->getName() + ".rewind", GamestationConf::getInstance()->get("global.rewind")) == "1");
    systemConfiguration->addWithLabel(rewind_enabled, _("REWIND"), _(MenuMessages::GAME_REWIND_HELP_MSG));
    // autosave
    auto autosave_enabled = std::make_shared<SwitchComponent>(mWindow);
    autosave_enabled->setState(GamestationConf::getInstance()->get(systemData->getName() + ".autosave", GamestationConf::getInstance()->get("global.autosave")) == "1");
    ...
    systemConfiguration->addSaveFunc(
            [systemData, smoothing_enabled, rewind_enabled, ratio_choice, emu_choice, core_choice, autosave_enabled] {
                if (ratio_choice->changed()) {
                    GamestationConf::getInstance()->set(systemData->getName() + ".ratio", ratio_choice->getSelected());
                }
                if (rewind_enabled->changed()) {
                    GamestationConf::getInstance()->set(systemData->getName() + ".rewind", rewind_enabled->getState() ? "1" : "0");
                }
                if (smoothing_enabled->changed()) {
                    GamestationConf::getInstance()->set(systemData->getName() + ".smooth", smoothing_enabled->getState() ? "1" : "0");
                }
                ...
                if (autosave_enabled->changed()) {
                    GamestationConf::getInstance()->set(systemData->getName() + ".autosave", autosave_enabled->getState() ? "1" : "0");
                }
                GamestationConf::getInstance()->saveGamestationConf();
            });
